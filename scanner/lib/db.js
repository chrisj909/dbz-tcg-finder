...
// jsonb needs an explicit stringify + cast; null when a source has no extras
// (most don't — only sources that capture more than one photo set this).
const imageUrlsJson = (l) => (l.image_urls && l.image_urls.length ? JSON.stringify(l.image_urls) : null)
  ...
  if (existing.length === 0) {
    await sql`
      INSERT INTO listings (
        ...
        quantity_available, product_type, set_name, condition, image_url, image_data, image_urls, seller,
        ...
        ${l.condition ?? null}, ${l.image_url ?? null}, ${l.image_data ?? null}, ${imageUrlsJson(l)}::jsonb, ${l.seller ?? null},
  ...
  if (priceChanged) {
    await sql`
      UPDATE listings SET
        ...
        image_urls = COALESCE(${imageUrlsJson(l)}::jsonb, image_urls),
